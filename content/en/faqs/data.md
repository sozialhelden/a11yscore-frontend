...
Tags allow computers to "understand" and filter the map. For example, to calculate how accessible a train station is, the a11y-Score algorithm looks for specific tags such as
```tactile_paving=yes``` (tactile paving available) or `wheelchair=yes` (place is wheelchair accessible).
